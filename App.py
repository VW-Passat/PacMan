    ...
    def playing_draw(self):
        ...
        for enemy in self.enemies:
            ...
            if self.player.grid_pos != self.last_player_pos:
                if self.path_algo_state == BFS_ALG:
                    enemy.path = enemy.breadth_first_search(vec(enemy.position), self.player.grid_pos)
                elif self.path_algo_state == DFS_ALG:
                    enemy.path = enemy.depth_first_search(vec(enemy.position), self.player.grid_pos)
                elif self.path_algo_state == UCS_ALG:
                    enemy.path = enemy.uniform_cost_search(vec(enemy.position), self.player.grid_pos)
